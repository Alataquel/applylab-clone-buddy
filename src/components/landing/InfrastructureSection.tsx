...
const StudentFeaturesContent = () => (
  <div className="space-y-32">
    {/* Feature 01 - Job Board */}
    <motion.div
    ...
    >
      <div>
        ...
        <p className="text-base text-gray-500 font-medium mb-3">AI-matched opportunities tailored to your skills and goals</p>
        <p className="text-sm text-gray-400 leading-relaxed">Our intelligent algorithm analyses your profile, skills, and preferences to surface the most relevant opportunities from thousands of listings — filtered by fit, not just keywords.</p>
      </div>
      <div className="bg-[#1a1f36] rounded-2xl p-5 shadow-2xl">
        ...
        <div className="space-y-3">
          {[
            ...
            <motion.div key={job.role} initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.3 + i * 0.1 }} className="flex items-center gap-3 bg-[#232942] rounded-xl px-4 py-3.5">
              <div className={`w-10 h-10 rounded-lg ${job.color} flex items-center justify-center text-xs font-bold text-white`}>{job.initials}</div>
              <div className="flex-1">
                <div className="flex items-center gap-1.5"><p className="text-sm font-medium text-white">{job.role}</p><ExternalLink className="w-3 h-3 text-white/40" /></div>
        ...
        <div className="flex items-center justify-between mt-4 px-1">
          ...
          <span className="text-xs text-primary font-medium flex items-center gap-1">View 47 more <ArrowRight className="w-3 h-3" /></span>
        </div>
      </div>
    </motion.div>

    {/* Feature 02 - Resume Lab */}
    <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6 }} className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
      <div className="order-2 lg:order-1 bg-[#1a1f36] rounded-2xl p-5 shadow-2xl">
        ...
        <div className="bg-[#232942] rounded-xl p-4 mb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full border-[3px] border-emerald-400 flex items-center justify-center"><span className="text-lg font-bold text-white">94</span></div>
              <div><p className="text-sm font-medium text-white">Resume Score</p><p className="text-xs text-emerald-400">Top 5% · Ready to send</p></div>
            </div>
        ...
        <div className="space-y-2.5">
          {[
            ...
            <motion.div key={item.text} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.3 + i * 0.1 }} className="flex items-center gap-3 bg-[#232942] rounded-xl px-4 py-3">
              <item.icon className={`w-4 h-4 ${item.color}`} /><p className="text-sm text-white/80">{item.text}</p>
            </motion.div>
          ))}
        </div>
      </div>
      <div className="order-1 lg:order-2">
        <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center mb-6"><FileText className="w-5 h-5 text-primary" /></div>
        ...
        <p className="text-base text-gray-500 font-medium mb-3">Real-time feedback to strengthen every application</p>
        <p className="text-sm text-gray-400 leading-relaxed">Build optimised resumes with instant feedback on formatting, keywords, and impact bullets. Our AI scores your resume against each specific job description.</p>
      </div>
    </motion.div>

    {/* Feature 03 - Automatic Tracker */}
    <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6 }} className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
      <div>
        <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center mb-6"><BarChart3 className="w-5 h-5 text-primary" /></div>
        ...
        <p className="text-base text-gray-500 font-medium mb-3">Every application, one visual pipeline</p>
        <p className="text-sm text-gray-400 leading-relaxed">A visual pipeline that automatically keeps every application organised. Track status in real time without lifting a finger — from application to offer.</p>
      </div>
      <div className="bg-[#1a1f36] rounded-2xl p-5 shadow-2xl">
        <div className="flex items-center gap-1.5 mb-5">
          <div className="w-3 h-3 rounded-full bg-red-400" /><div className="w-3 h-3 rounded-full bg-yellow-400" /><div className="w-3 h-3 rounded-full bg-green-400" />
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            <motion.div key={col.stage} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.3 + colIdx * 0.08 }}>
              <div className="flex items-center gap-1.5 mb-2"><div className={`w-2 h-2 rounded-full ${col.color}`} /><span className="text-[10px] font-medium text-white">{col.stage}</span></div>
              <div className="space-y-1.5">
                {col.items.map((item) => (<div key={item} className="bg-[#232942] rounded-lg px-2.5 py-2"><p className="text-[10px] font-medium text-white/80">{item}</p></div>))}
                {col.items.length === 0 && (<div className="border border-dashed border-white/10 rounded-lg px-2.5 py-2 text-center"><p className="text-[10px] text-white/20">—</p></div>)}
              </div>
            </motion.div>
          ))}
        </div>
    ...
    <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6 }} className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
      <div className="order-2 lg:order-1 bg-[#1a1f36] rounded-2xl p-5 shadow-2xl">
        ...
        <div className="space-y-2.5">
          {[
            ...
            <motion.div key={event.title} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.3 + i * 0.1 }} className="flex items-center gap-3 bg-[#232942] rounded-xl px-4 py-3">
              ...
              <div className="flex-1"><p className="text-sm font-medium text-white">{event.title}</p><p className="text-xs text-white/50">{event.org}</p></div>
      ...
      <div className="order-1 lg:order-2">
        <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center mb-6"><Calendar className="w-5 h-5 text-primary" /></div>
        ...
        <p className="text-base text-gray-500 font-medium mb-3">Never miss a career opportunity</p>
        <p className="text-sm text-gray-400 leading-relaxed">Discover career fairs, networking sessions, and workshops curated for your profile. Never miss an opportunity to connect with employers.</p>
      </div>
    </motion.div>

    {/* Feature 05 - Book an Advisor */}
    <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6 }} className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
      <div>
        <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center mb-6"><UserCheck className="w-5 h-5 text-primary" /></div>
        ...
        <p className="text-base text-gray-500 font-medium mb-3">Personalised guidance, one click away</p>
        <p className="text-sm text-gray-400 leading-relaxed">One-click booking with your university's career advisors. Get personalised guidance on your applications, interview prep, or career direction.</p>
      </div>
      <div className="bg-[#1a1f36] rounded-2xl p-5 shadow-2xl">
        <div className="flex items-center gap-1.5 mb-5">
          <div className="w-3 h-3 rounded-full bg-red-400" /><div className="w-3 h-3 rounded-full bg-yellow-400" /><div className="w-3 h-3 rounded-full bg-green-400" />
        ...
        <div className="space-y-2.5">
          {[
            ...
            <motion.div key={advisor.name} initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.3 + i * 0.1 }} className="flex items-center gap-3 bg-[#232942] rounded-xl px-4 py-3">
              <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center"><UserCheck className="w-4 h-4 text-primary" /></div>
              <div className="flex-1"><p className="text-sm font-medium text-white">{advisor.name}</p><p className="text-xs text-white/50">{advisor.role}</p></div>
              <div className="text-right"><p className="text-[10px] text-emerald-400">{advisor.availability}</p><p className="text-[10px] text-white/30">{advisor.slots}</p></div>
            </motion.div>
          ))}
        </div>
      </div>
...
const InfrastructureSection = () => {
  const [activeView, setActiveView] = useState<"university" | "student">("university");
    ...
    <section className="py-24 px-6 lg:px-12 bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="max-w-2xl mb-8">
            ...
            <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 tracking-tight mb-4">
              {activeView === "university" ? "Your institutional dashboard." : "Your career toolkit."}
            </h2>
            <p className="text-sm text-gray-500 leading-relaxed">
              {activeView === "university"
                ...
                : "Everything you need to apply with precision and land with confidence — AI job matching, resume tools, application tracking, and more."}
            </p>
          </div>
          <div className="inline-flex items-center bg-gray-100 rounded-full p-1 gap-1">
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                activeView === "university" ? "bg-gray-900 text-white shadow-md" : "text-gray-500 hover:text-gray-700"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                activeView === "student" ? "bg-gray-900 text-white shadow-md" : "text-gray-500 hover:text-gray-700"
              }`}
            >
        ...
        <AnimatePresence mode="wait">
          {activeView === "university" ? (
            <motion.div key="university" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}>
              <div className="space-y-32">
